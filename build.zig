...
pub fn build(b: *std.Build) !void {
    ...
    std.debug.print("Copied rust-lib.so to {s}\n", .{ZIG_BIN_OUT_DIR});

    const target = b.standardTargetOptions(.{});

    const optimize = b.standardOptimizeOption(.{});

    const exe = b.addExecutable(.{ .name = "zig-exe", .root_source_file = b.path("src/main.zig"), .target = target, .optimize = optimize, .link_libc = true });
    exe.linkSystemLibrary("stdc++");
    exe.addObjectFile(b.path("zig-out/bin/librust_libp2p.a"));
